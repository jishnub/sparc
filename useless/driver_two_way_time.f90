    ...
    leng = 2*halftime+1
     ...
     do i=1,nx
      if ((distances(i) > mindist) .and. (distances(i) < maxdist)) then
       ...
       lef = loc - halftime
       rig = loc + halftime
       call compute_tt(real(acc(i,1,lef:rig)),real(dat(i,1,lef:rig)),taus(i),dt,leng)
      endif
     enddo
! HERE !
     call dbsnak(nx,signed,kxord,xknot)
     call dbsint(nx,signed,taus,kxord,xknot,bcoef)
     ...
     do i=1,nx
      if ((distances(i) > mindist) .and. (distances(i) < maxdist)) then
       ...
       rig = loc + halftime
       con = -signed(i)
       if ((con .gt. signed(1)) .and. (con .lt. signed(nx))) then
        tau = dbsval(con,kxord,xknot,nx,bcoef)
        tau = tau * sign(1.0,con) + taus(i)*sign(1.0, signed(i))
       else
        tau = 0.0
       endif 
